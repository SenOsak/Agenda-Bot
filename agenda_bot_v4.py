"""
╔══════════════════════════════════════════════════════╗
║       BOT DE AGENDA DIARIA v4 - TELEGRAM             ║
║                                                      ║
║  Todo lo de v3 más:                                  ║
║   🤖 Lenguaje natural con Gemini AI                  ║
║   💬 Escríbele como si fuera una persona             ║
║                                                      ║
...
#  Para correr local crea un archivo .env o setea las variables
# ─────────────────────────────────────────
import os
...
ARCHIVO_REPETIDAS  = "repetidas.json"
# ─────────────────────────────────────────
...
def guardar_json(archivo: str, datos: dict):
...
def obtener_tareas(fecha: str = None) -> list:
    agenda = cargar_json(ARCHIVO_AGENDA)
    return agenda.get(fecha or obtener_fecha(), [])
...
def guardar_tareas(tareas: list, fecha: str = None):
    agenda = cargar_json(ARCHIVO_AGENDA)
    agenda[fecha or obtener_fecha()] = tareas
...
def agregar_tarea(texto: str, prioridad: str = "normal", hora: str = None, fecha: str = None) -> dict:
    ...
    tareas.append(tarea)
    guardar_tareas(tareas, fecha)
    return tarea


def completar_tarea(numero: int) -> bool:
...
def eliminar_tarea(numero: int) -> bool:
    tareas = obtener_tareas()
    nuevas = [t for t in tareas if t["id"] != numero]
...
def mover_tarea(numero: int, fecha_destino: str) -> bool:
    tareas_hoy = obtener_tareas()
    tarea = next((t for t in tareas_hoy if t["id"] == numero), None)
    if not tarea:
        return False
    tareas_hoy = [t for t in tareas_hoy if t["id"] != numero]
    guardar_tareas(tareas_hoy)
    tareas_destino = obtener_tareas(fecha_destino)
    tarea["id"] = siguiente_id(tareas_destino)
    tarea["recordatorio_enviado"] = False
    tareas_destino.append(tarea)
    guardar_tareas(tareas_destino, fecha_destino)
    return True


def cargar_repetidas() -> list:
    datos = cargar_json(ARCHIVO_REPETIDAS)
    return datos.get("tareas", [])


def agregar_repetida(texto: str, prioridad: str = "normal", hora: str = None):
    datos = cargar_json(ARCHIVO_REPETIDAS)
    repetidas = datos.get("tareas", [])
    repetidas.append({"texto": texto, "prioridad": prioridad, "hora": hora})
    datos["tareas"] = repetidas
    guardar_json(ARCHIVO_REPETIDAS, datos)


def inyectar_repetidas():
    repetidas = cargar_repetidas()
    if not repetidas:
        return
    tareas = obtener_tareas()
    textos_existentes = {t["texto"] for t in tareas}
    for r in repetidas:
...
def consultar_gemini(mensaje_usuario: str, tareas_hoy: list) -> dict:
    """
    Llama a Gemini para interpretar el mensaje en lenguaje natural.
    ...
    if tareas_hoy:
        for t in tareas_hoy:
            ...
            tareas_str += f"  ID {t['id']}: {estado} {t['texto']} (prioridad: {t['prioridad']}, hora: {t.get('hora','sin hora')})\n"
    else:
...
Analiza el mensaje y responde SOLO con un JSON válido (sin texto extra, sin markdown, sin explicaciones) con esta estructura:
...
{{
  ...
  "hora": "HH:MM o null",
  "fecha": "YYYY-MM-DD o null (para agregar en fecha específica o mover)",
  "id": numero o null (para completar, eliminar, mover),
  "respuesta": "mensaje amigable confirmando lo que harás"
}}

Reglas:
- Si el usuario dice "mañana" usa la fecha {manana}
- Si dice "urgente", "importante", "crítico" → prioridad urgente
- Si dice "cuando pueda", "sin prisa", "baja" → prioridad baja
- Si no se entiende bien qué tarea completar/eliminar, usa accion "chat" y pregunta
- Para saludos, preguntas generales o conversación usa accion "chat"
- La respuesta debe ser en español, amigable y corta (máx 2 líneas)
- Si agregar una tarea, confirma con el nombre de la tarea y la hora si la tiene"""
...
async def procesar_lenguaje_natural(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja mensajes de texto libre usando Gemini."""
    ...
    resultado = consultar_gemini(mensaje, tareas)
    ...
    if accion == "agregar":
        ...
        msg       = f"{emoji} {respuesta}\n\n_Tarea #{tarea['id']} guardada_{hora_txt}"
        ...
        if id_tarea and completar_tarea(int(id_tarea)):
            ...
            await update.message.reply_text(f"❌ No encontré esa tarea. Usa /agenda para ver los números.")
        ...
        if id_tarea and eliminar_tarea(int(id_tarea)):
            ...
            await update.message.reply_text(f"❌ No encontré esa tarea.")
        ...
        if id_tarea and fecha and mover_tarea(int(id_tarea), fecha):
            ...
            await update.message.reply_text(f"❌ No pude mover la tarea.")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "👋 *¡Hola! Soy tu asistente de agenda v4 con IA.*\n\n"
        "Ahora puedes escribirme en lenguaje natural:\n"
        "  _\"recuérdame llamar al médico mañana a las 10\"_\n"
        "  _\"marca la tarea 2 como completada\"_\n"
        "  _\"mueve la reunión a el jueves\"_\n"
        "  _\"¿qué tengo hoy?\"_\n\n"
        "O usa los comandos de siempre:\n"
...
def job_recordatorios(app):
    ...
    modificado = False
    for t in tareas:
        if t.get("hora") and not t["hecho"] and not t.get("recordatorio_enviado"):
            if t["hora"] == hora_30:
                msg = (
                    f"⏰ *Recordatorio*\n\n"
...
def main():
    print("🤖 Iniciando bot de agenda v4 con IA...")
    ...
    # Lenguaje natural — captura todos los mensajes de texto
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, procesar_lenguaje_natural))

    iniciar_scheduler(app)
    print("✅ Bot corriendo con IA. Presiona Ctrl+C para detener.")
